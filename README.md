...
The module also comes ready-made: `sudo apt install libsmspp-mmcf-dev` from the
[PPA of the project](https://launchpad.net/~smspp/+archive/ubuntu/ppa), which
has `smspp-mmcf` for its command-line tool as well, and `vcpkg install
"smspp[core,mmcf]"` from the [SMS++ vcpkg
